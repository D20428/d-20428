...
# --------------------------------------------------
# [구역 3] 다중 선그래프 - 상위 5개 영화 누적관객수 비교
# --------------------------------------------------
st.subheader("3. TOP 5 영화 누적 관객수 비교")

# 누적관객수 상위 5개 영화 목록 추출 (movie_order는 이미 내림차순 정렬되어 있음)
top5_movies = movie_order[:5]

# 상위 5개 영화의 데이터만 추출
top5_df = df[df["영화명"].isin(top5_movies)]

# Plotly 다중 선그래프 생성 (color="영화명"을 지정하여 영화별 색상과 범례 구분)
fig3 = px.line(
    top5_df,
    x="기준일자",
    y="누적관객수",
    color="영화명",
    title=f"누적 관객수 TOP 5 영화 비교 ({', '.join(top5_movies[:3])} 등)",
    markers=True,
...
# 알 수 있는 것 문구
st.info("💡 **이 그래프로 알 수 있는 것:** 전체 기간 누적 관객수가 가장 높은 상위 5개 흥행작들의 성장 곡선을 서로 비교하여, 초기 흥행 속도와 최종 관객수 차이를 한눈에 파악할 수 있습니다.")
